...
goal_position = {goal_config[row][col]: (
    ...
    def solve(self, parent_state: State) -> bool:
        ...
        for direction, (offset_row, offset_col) in operators.items():
            ...
            if self.is_valid_move(next_row, next_col):
                ...
                g_current, h_current = g_parent + \
                    1, self.calculate_manhattan_distance(board_parent)

                # Make next State object and add node to the solution graph
                next_state = State(
                    g=g_current, h=h_current, parent=parent_state, board=deepcopy(board_parent))
                self.graph.add_node(next_state.node)

                # Draw edge from parent node to next generated node
                edge = pydot.Edge(
                    parent_state.node_name, next_state.node_name, label=f" {direction}", fontsize="24")
